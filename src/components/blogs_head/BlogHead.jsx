...
const BlogHead = () => {
   ...
   useEffect(() => {
    ...
    const signal = controller.signal;
     FetchData(`/api/v1/user/get/user?userId=${userId}`,{signal})
  ...
  return (
    ...
    <div className="flex justify-between items-center ">
      <div className="flex">
       ...
       <h1 className="text-start text-gray-200 text-[18px] font-semibold">{user?.username}</h1>
       <p className="text-start text-gray-300 text-[12px] font-normal">{user?.followers} • followers</p>
       </div>
      </div>
      <div>
       <ButtonWhite
   ...
   <div className="flex justify-start flex-wrap mt-3">
   {
    ...
    <p className=" text-start text-gray-200 font-normal text-sm  mt-3 leading-[1.1]">{user?.bio}</p>
    </div>
  )
}
